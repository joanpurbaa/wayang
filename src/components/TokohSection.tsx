...
function InteractiveModel({
	...
	const getIntersectedPart = useCallback(
		(ndc: THREE.Vector2): string | null => {
			...
			const meshes = [headRef.current, bodyRef.current, lowerRef.current].filter(
				Boolean,
			) as THREE.Object3D[];
...
function TokohCard({ t, index }: { t: any; index: number }) {
	...
	const partDescription = activePart
		? t.partsDescriptions[activePart as keyof typeof t.partsDescriptions]
		: null;
