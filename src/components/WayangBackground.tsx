...
function WayangModel() {
  const outerGroupRef = useRef<THREE.Group>(null!); // Gerakan GSAP (posisi, rotasi, skala)
  const innerGroupRef = useRef<THREE.Group>(null!); // Auto‑rotate konstan
  ...
  // Normalisasi posisi & material premium
  useEffect(() => {
    ...
    // Posisi awal close‑up (separuh badan atas)
  ...
  // Scroll global: transisi posisi & skala untuk narasi visual
  useEffect(() => {
    const proxy = { p: 0 };
    ...
    // Bagian 1: Landing → tengah halaman (0 → 0.5)
    tl.to(proxy, {
      p: 0.5,
      onUpdate() {
        ...
        outerGroupRef.current.position.y = lerp(
          isMobile ? -1.5 : -2.2,
          -0.2,
          e
        );
    ...
    // Bagian 2: Tengah → akhir halaman (0.5 → 1)
    tl.to(proxy, {
      p: 1,
      onUpdate() {
        if (!outerGroupRef.current) return;
...
// ─── Komponen Utama dengan Overlay Vignette ──────────────────────────────────
export default function WayangBackground() {
  return (
    <>
      ...
      >
        ...
        >
          ...
          <directionalLight
            ...
            shadow-bias={-0.0001}
          />
          {/* Rim light neon emas belakang */}
          <directionalLight position={[-5, 3, -5]} intensity={4.5} color="#ffbd3d" />
          {/* Fill light dingin bawah */}
          <directionalLight position={[-4, -2, 3]} intensity={1.0} color="#a2b3cb" />
          {/* Accent spot depan */}
          <pointLight position={[0, 0, 4]} intensity={2.0} color="#ffffff" decay={1.5} />
          {/* Environment studio untuk refleksi material clearcoat */}
      ...
      {/* Overlay vignette – fokus total ke wayang */}
      <div
        style={{
          position: "fixed",
          inset: 0,
